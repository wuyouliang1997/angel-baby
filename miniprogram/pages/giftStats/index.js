...
Page({
  ...
  async loadStats() {
    ...
    const res = await this.callCloudFunction('getGiftRecords', {
      babyId: this.data.babyId, page: 1, pageSize: 1000,
    ...
    if (res && res.success) {
      // 日期已在服务端过滤
      let records = res.data.slice();

      // 按来源人或说明搜索
      if (this.data.searchText) {
        const kw = this.data.searchText.toLowerCase();
        records = records.filter(r =>
          (r.source && r.source.toLowerCase().includes(kw)) ||
      ...
      this.setData({ records: paged, totalAmount, totalCount: records.length, totalPages });
    }
  },
  ...
  onFilterChange() { this.setData({ currentPage: 1 }); this.loadStats(); },
